...
const { getDb, queryDb, queryDbOne, execDb, transactionDb, batchInsertDb } = require('./db');
...
class ScanProcessor {
  ...
  /**
   * Flush buffered files to database in a transaction
   */
    ...
    try {
      await transactionDb(async () => {
        ...
        const values = this.fileBuffer.flatMap(f => [
          ...
          f.ext, f.category, f.is_duplicate, f.duplicate_group_id,
